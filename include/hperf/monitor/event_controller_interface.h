...
class EventControllerInterface {
 public:
  ...
  virtual std::error_code control(unsigned long request, void* arg) const = 0;
  ...
  virtual std::optional<uint64_t> time_enabled() const = 0;
  virtual std::optional<uint64_t> time_running() const = 0;
