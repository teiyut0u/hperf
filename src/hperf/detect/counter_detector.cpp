...
#include "hperf/util/read_buffer.h"

hperf::CounterDetector::CounterDetector()
    : detected_(false),
      fds_(),
...
void hperf::CounterDetector::detect() {
...
bool hperf::CounterDetector::test(uint64_t cpu_id, uint64_t event_num) {
...
bool hperf::CounterDetector::enable_all_events() {
...
bool hperf::CounterDetector::disable_all_events() {
  for (const int fd : fds_) {
...
void hperf::CounterDetector::close_all_events() {
  for (int fd : fds_) {
    if (fd != -1) {
      close(fd);
    }
...
int hperf::CounterDetector::get_detected_general_counter_num(uint64_t cpu_id) const {
  if (!detected_) {
...
int hperf::CounterDetector::get_detected_general_counter_num() const {
  if (!detected_) {
    std::cerr << "The number of avaliable programmable counters is undetected" << std::endl;
    return -1;
  }
...
void hperf::CounterDetector::print_result() const {
  if (!detected_) {
    std::cerr << "The number of avaliable programmable counters is undetected" << std::endl;
    return;
  }
...
void hperf::CounterDetector::configure_event(struct perf_event_attr* pe,
                                             uint64_t encoding) {
...
int hperf::CounterDetector::perf_event_open(struct perf_event_attr* pe,
                                            int cpu_id) {
...
bool hperf::CounterDetector::load_detected_result() {
  std::ifstream infile(HperfCache::COUNTER_CACHE());
  if (!infile.is_open()) {
    return false;
  }
