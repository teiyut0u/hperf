...
hperf::SingleEventController::SingleEventController() {
...
hperf::SingleEventController::SingleEventController(SingleEventController&& another) noexcept {
  this->time_enabled_offset_ = 8;
  this->fd_ = -1;
  buffer_.reserve(40);
...
hperf::SingleEventController& hperf::SingleEventController::operator=(SingleEventController&& another) noexcept {
...
std::error_code hperf::SingleEventController::open_event(perf_event_attr* attr_ptr, pid_t pid, int cpu, unsigned long flags) {
  // open event
  ...
  if ((this->fd_ = perf_event_open(attr_ptr, pid, cpu, -1, flags)) == -1) {
...
std::error_code hperf::SingleEventController::read_event() {
...
std::error_code hperf::SingleEventController::close_event() {
  if (this->fd_ == -1) {
    return std::error_code{};
  }
  int old_fd = this->fd_;
...
// size_t hperf::SingleEventController::size() const {
//   return this->buffer_.size();
// }

uint64_t hperf::SingleEventController::value() const {
  return *reinterpret_cast<const uint64_t*>(this->buffer_.data());
}

std::optional<uint64_t> hperf::SingleEventController::time_enabled() const {
  if (this->read_format_ & PERF_FORMAT_TOTAL_TIME_ENABLED) {
...
std::optional<uint64_t> hperf::SingleEventController::time_running() const {
  if (this->read_format_ & PERF_FORMAT_TOTAL_TIME_RUNNING) {
...
std::optional<uint64_t> hperf::SingleEventController::id() const {
  if (this->read_format_ & PERF_FORMAT_ID) {
...
std::optional<uint64_t> hperf::SingleEventController::lost() const {
  if (this->read_format_ & PERF_FORMAT_LOST) {
